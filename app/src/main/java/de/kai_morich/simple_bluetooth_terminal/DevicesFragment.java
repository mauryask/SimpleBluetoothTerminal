...
public class DevicesFragment extends ListFragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (Objects.requireNonNull(requireActivity()).getPackageManager().hasSystemFeature(PackageManager.FEATURE_BLUETOOTH))
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        setListAdapter(null);
        View header = LayoutInflater.from(requireActivity()).inflate(R.layout.device_list_header, getListView(), false);
        getListView().addHeaderView(header, null, false); // Add header before setting adapter
        setEmptyText("initializing...");
        View emptyView = getListView().getEmptyView();
        if (emptyView instanceof TextView) {
            ((TextView) emptyView).setTextSize(18);
        }
